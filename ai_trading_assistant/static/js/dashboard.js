...
// LIVE INDICATORS: Track previous values for change detection
...
// LIVE INDICATORS: Auto-refresh interval
let indicatorsAutoRefresh = null;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // LIVE INDICATORS: Start auto-refresh (every 30 seconds)
    startIndicatorsAutoRefresh();
    ...
    console.log('✅ Periodic price updates enabled (every 30 seconds)');
...
function initDashboard() {
    ...
    if (refreshBtn) {
        ...
        console.log('✅ Prediction button connected');
    }
    ...
    if (buyBtn) {
        ...
        console.log('✅ Buy button connected');
    }
    
    if (sellBtn) {
        sellBtn.addEventListener('click', () => handleTrade('SELL'));
        console.log('✅ Sell button connected');
    }
    ...
    if (quantityInput) {
        ...
        console.log('✅ Quantity input connected');
    }
    ...
    if (symbolSelect) {
        ...
        console.log('✅ Trading symbol selector connected');
    }
    ...
    if (refreshIndicatorsBtn) {
        refreshIndicatorsBtn.addEventListener('click', function(e) {
            console.log('🔄 Refresh indicators clicked');
            loadIndicators();
        });
        console.log('✅ Indicators refresh button connected');
    } else {
        console.log('⚠️ Indicators refresh button not found');
    }
    ...
    if (syncPricesBtn) {
        ...
        console.log('✅ Sync prices button connected');
    }
    ...
    if (toggleFormBtn) {
        toggleFormBtn.addEventListener('click', function(e) {
            ...
            console.log('🤖 Toggle grid bot form clicked');
            toggleGridBotForm();
        });
        console.log('✅ Grid bot toggle button connected');
    } else {
        console.log('⚠️ Grid bot toggle button not found');
    }
    ...
    if (createBotForm) {
        ...
        console.log('✅ Grid bot form connected to handleCreateGridBot');
        console.log('📋 Form element:', createBotForm);
        ...
        if (allFieldsPresent) {
            console.log('🎉 All grid bot form fields are present and ready!');
        } else {
            console.log('⚠️ Some grid bot form fields are missing!');
        }
    } else {
        console.log('⚠️ Grid bot form not found');
    }
    ...
    if (closeGridLevelsBtn) {
        ...
        console.log('✅ Close grid levels button connected');
    }
    ...
    if (gridSymbolSelect) {
        ...
        console.log('✅ Grid Bot symbol selector connected');
    }
    ...
    if (dcaSymbolSelect) {
        ...
        console.log('✅ DCA Bot symbol selector connected');
    }
    ...
    if (createDCABotForm) {
        ...
        console.log('✅ DCA bot form connected to handleCreateDCABot');
        console.log('📋 Form element:', createDCABotForm);
        ...
        if (allDCAFieldsPresent) {
            console.log('🎉 All DCA bot form fields are present and ready!');
        } else {
            console.log('⚠️ Some DCA bot form fields are missing!');
        }
    } else {
        console.log('⚠️ DCA bot form not found');
    }
    ...
    if (exchangeAccountSelect) {
        ...
        console.log('✅ Exchange account selector connected');
    }
    ...
    if (refreshExchangeBtn) {
        ...
        console.log('✅ Refresh exchange button connected');
    }
    ...
    if (aiTradeBtn) {
        ...
        console.log('✅ AI Trade button connected');
    }
    ...
    if (gridTypeSelect) {
        ...
        console.log('✅ Grid Type selector connected');
    }
    ...
    try {
        ...
        contentDiv.innerHTML = `
            <div class="prediction-empty" style="color: var(--danger-color);">
                <p><strong>❌ Failed to generate prediction</strong></p>
                <p>${error.message}</p>
                <p style="font-size: 0.875rem; margin-top: 1rem;">
                    Please make sure:<br>
                    • Model is trained (run: python services/train_model.py)<br>
                    • Price data exists in database<br>
...
function updatePredictionDataInfo(data) {
    ...
    console.log('✅ Prediction data freshness info updated');
}
...
async function updatePriceDisplay() {
    ...
    try {
        ...
        if (data.success && data.price) {
            ...
            console.log(`✅ Updated Buy/Sell price: ${symbol} = ${data.formatted}`);
...
async function updateGridPrice() {
    ...
    try {
        ...
        if (data.success && data.price) {
            ...
            console.log(`✅ Updated Grid Bot price: ${symbol} = ${data.formatted}`);
        } else {
        ...
        console.error('❌ Error fetching grid price:', error);
        priceDisplay.textContent = 'Error loading price';
...
async function updateDCAPrice() {
    ...
    try {
        ...
        if (data.success && data.price) {
            ...
            console.log(`✅ Updated DCA Bot price: ${symbol} = ${data.formatted}`);
        } else {
            priceDisplay.textContent = 'Error loading price';
            priceDisplay.style.color = '#ef4444'; // Red color
        }
...
async function loadIndicators() {
    ...
    try {
        ...
        // Fetch indicators from backend (includes LIVE Binance price via CCXT)
        const indicatorsResponse = await fetch('/api/indicators/' + activeSymbol);
        const data = await indicatorsResponse.json();
        
        console.log('   Indicators data:', data);
        
        if (data.success) {
            // ✅ Using LIVE price from Binance via CCXT
            // The current_price in the response is already the latest from Binance
            // No need to fetch from CoinMarketCap - we have real-time data!
            console.log(`✅ Using LIVE price from Binance via CCXT: $${data.current_price}`);
...
/**
 * Display technical indicators in the UI (ENHANCED with live updates & animations)
 * @param {Object} data - Indicators data from API
 */
function displayIndicators(data) {
    console.log('📈 Displaying indicators...');
    ...
    if (currentPriceEl && data.current_price) {
        const prevPrice = previousIndicators.currentPrice;
        ...
        if (prevPrice !== null && prevPrice !== newPrice) {
            flashElement(currentPriceEl, newPrice > prevPrice ? 'green' : 'red');
        }
        
        previousIndicators.currentPrice = newPrice;
        console.log(`   Current Price: $${newPrice}`);
    }
    
    // TASK 40: Update MA 9 with animation
    const ema9El = document.getElementById('ma9Value');
    if (ema9El) {
        if (data.ma9) {
            const prevMA9 = previousIndicators.ma9;
            const newMA9 = data.ma9;
            
            ema9El.textContent = 
                '$' + newMA9.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2});
            
            if (prevMA9 !== null && prevMA9 !== newMA9) {
                flashElement(ema9El, newMA9 > prevMA9 ? 'green' : 'red');
            }
            
            previousIndicators.ma9 = newMA9;
            console.log(`   MA 9: $${newMA9}`);
        } else {
            ema9El.textContent = 'N/A';
        }
    }
    
    // TASK 40: Update MA 20 with animation
    const ema20El = document.getElementById('ma20Value');
    if (ema20El) {
        if (data.ma20) {
            const prevMA20 = previousIndicators.ma20;
            const newMA20 = data.ma20;
            
            ema20El.textContent = 
                '$' + newMA20.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2});
            
            if (prevMA20 !== null && prevMA20 !== newMA20) {
                flashElement(ema20El, newMA20 > prevMA20 ? 'green' : 'red');
            }
            
            previousIndicators.ma20 = newMA20;
            console.log(`   MA 20: $${newMA20}`);
        } else {
            ema20El.textContent = 'N/A';
        }
    }
    
    // TASK 40: Update MA 50 with animation
    const ema50El = document.getElementById('ma50Value');
    if (ema50El) {
        if (data.ma50) {
            const prevMA50 = previousIndicators.ma50;
            const newMA50 = data.ma50;
            
            ema50El.textContent = 
                '$' + newMA50.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2});
            
            if (prevMA50 !== null && prevMA50 !== newMA50) {
                flashElement(ema50El, newMA50 > prevMA50 ? 'green' : 'red');
            }
            
            previousIndicators.ma50 = newMA50;
            console.log(`   MA 50: $${newMA50}`);
        } else {
            ema50El.textContent = 'N/A';
        }
    }
    
    // TASK 40: Update MA 200 with animation
    const ema200El = document.getElementById('ma200Value');
    if (ema200El) {
        if (data.ma200) {
            const prevMA200 = previousIndicators.ma200;
            const newMA200 = data.ma200;
            
            ema200El.textContent = 
                '$' + newMA200.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2});
            
            if (prevMA200 !== null && prevMA200 !== newMA200) {
                flashElement(ema200El, newMA200 > prevMA200 ? 'green' : 'red');
            }
            
            previousIndicators.ma200 = newMA200;
            console.log(`   MA 200: $${newMA200}`);
        } else {
            ema200El.textContent = 'N/A';
        }
    }
    
    // Update RSI 14 with color coding and animation
    ...
    if (rsiElement && data.rsi14 !== null && data.rsi14 !== undefined) {
        const prevRSI = previousIndicators.rsi14;
        const newRSI = data.rsi14;
        
        rsiElement.textContent = newRSI.toFixed(1);
        
        // Flash animation if RSI changed significantly (more than 0.5)
        if (prevRSI !== null && Math.abs(prevRSI - newRSI) > 0.5) {
            flashElement(rsiElement, newRSI > prevRSI ? 'green' : 'red');
        }
        
        previousIndicators.rsi14 = newRSI;
        console.log(`   RSI 14: ${newRSI.toFixed(1)}`);
    ...
    if (trendSignalElement && data.sma_trend) {
        if (data.sma_trend === 'Bullish') {
            trendSignalElement.textContent = '🟢 Bullish trend';
            trendSignalElement.style.color = 'var(--success-color)';
            console.log('   → Bullish trend (SMA20 > SMA50)');
        } else {
            trendSignalElement.textContent = '🔴 Bearish trend';
            trendSignalElement.style.color = 'var(--danger-color)';
            console.log('   → Bearish trend (SMA20 < SMA50)');
        }
    }
    ...
    displayEMASignals(data.ma_signals);
    
    console.log('✅ Indicators displayed successfully!');
}
...
function displayEMASignals(signals) {
    if (!signals) {
        console.log('⚠️ No EMA signals data available');
        return;
    }
    
    console.log('⚡ Displaying EMA signals...', signals);
    
    // Overall Signal with color coding
    const overallSignalEl = document.getElementById('maOverallSignal');
    if (overallSignalEl) {
    ...
    // Long-term Trend
    const trendLabelEl = document.getElementById('maTrendLabel');
    if (trendLabelEl) {
        if (signals.trend_label === 'long_term_uptrend') {
            trendLabelEl.textContent = '📈 Uptrend';
            trendLabelEl.style.color = '#10b981';
        } else if (signals.trend_label === 'long_term_downtrend') {
            trendLabelEl.textContent = '📉 Downtrend';
    ...
    // Short-term
    const shortTermEl = document.getElementById('maShortTerm');
    if (shortTermEl) {
        if (signals.short_term === 'bullish') {
            shortTermEl.textContent = '🔥 Bullish';
            shortTermEl.style.color = '#10b981';
        } else if (signals.short_term === 'bearish') {
            shortTermEl.textContent = '❄️ Bearish';
    ...
    if (crossInfoEl) {
        if (signals.golden_cross) {
            crossInfoEl.textContent = '🌟 GOLDEN CROSS! Major bullish signal (EMA50 crossed above EMA200)';
            ...
            crossInfoEl.textContent = '💀 DEATH CROSS! Major bearish signal (EMA50 crossed below EMA200)';
    ...
    // Explanation
    const explanationEl = document.getElementById('maExplanation');
    ...
    console.log('✅ EMA signals displayed successfully!');
}
...
function updateIndicatorDataInfo(data) {
    ...
    console.log('✅ Indicator data freshness info updated');
}
 ...
 */
// ============================================
// LIVE INDICATORS: AUTO-REFRESH SYSTEM
// ============================================

/**
 * Start auto-refresh for technical indicators (every 30 seconds)
 */
function startIndicatorsAutoRefresh() {
    // Clear any existing interval
    if (indicatorsAutoRefresh) {
        clearInterval(indicatorsAutoRefresh);
    }
    
    // Start new interval - refresh every 30 seconds
    indicatorsAutoRefresh = setInterval(() => {
        console.log('🔄 Auto-refreshing indicators (30s interval)...');
        loadIndicators();
        updateIndicatorTimestamp();
    }, 30000); // 30 seconds
    
    console.log('✅ Live indicators auto-refresh started (every 30 seconds)');
}

/**
 * Stop auto-refresh for technical indicators
 */
function stopIndicatorsAutoRefresh() {
    if (indicatorsAutoRefresh) {
        clearInterval(indicatorsAutoRefresh);
        indicatorsAutoRefresh = null;
        console.log('⏸️  Live indicators auto-refresh stopped');
    }
}
 ...
 * @param {string} color - 'green' for increase, 'red' for decrease
 */
...
/**
 * Update the indicator last update timestamp
 */
function updateIndicatorTimestamp() {
    const timestampEl = document.getElementById('indicatorLastUpdate');
    if (timestampEl) {
        const now = new Date();
...
function displayIndicatorsError(errorMessage = null) {
    ...
    document.getElementById('ma9Value').textContent = 'N/A';
    document.getElementById('ma20Value').textContent = 'N/A';
    ...
    document.getElementById('maOverallSignal').textContent = '--';
    document.getElementById('maConfidence').textContent = '--';
    document.getElementById('maTrendLabel').textContent = '--';
    document.getElementById('maShortTerm').textContent = '--';
    document.getElementById('maExplanation').textContent = 'No data available';
        ...
        banner.innerHTML = `
            <div style="display: flex; align-items: start; gap: 0.75rem;">
                ...
                <div style="flex: 1;">
                    ...
                    <div style="opacity: 0.95;">${errorMessage || 'Not enough historical data to calculate indicators (need at least 200 candles for MA 200).'}</div>
...
function toggleGridBotForm() {
    ...
    if (form.style.display === 'none' || form.style.display === '') {
        ...
        button.textContent = '➕ Create New Bot';
    ...
    bots.forEach(bot => {
        ...
        row.innerHTML = `
            ...
            <td onclick="event.stopPropagation();" style="display: flex; gap: 0.5rem;">
                ${bot.is_active === 1 ? 
                    `<button class="btn btn-sm btn-danger" onclick="stopGridBot(${bot.id})" title="Stop bot">⏹️ Stop</button>` : 
                    `<button class="btn btn-sm btn-danger" onclick="deleteGridBot(${bot.id})" title="Delete bot">🗑️ Delete</button>`
                }
            </td>
        `;
...
async function handleCreateGridBot(e) {
    ...
    console.log('🤖 Grid Bot Creation Started (Binance-Style)!');
    console.log('📋 Form submitted, preparing to create grid bot...');
    ...
    if (lowerPrice >= upperPrice) {
        console.log('❌ Validation failed: Upper price must be greater than lower price');
        showNotification('❌ Upper price must be greater than lower price', 'error');
        return;
    }
    
    if (gridCount < 2 || gridCount > 100) {
        console.log('❌ Validation failed: Grid count must be between 2 and 100');
        showNotification('❌ Grid count must be between 2 and 100', 'error');
        return;
    }
    
    if (investment <= 0) {
        console.log('❌ Validation failed: Investment must be greater than 0');
        showNotification('❌ Investment must be greater than 0', 'error');
        return;
    }
    ...
    try {
        ...
        console.log('📡 Sending POST request to /api/grid_bot/create...');
        ...
        if (data.success) {
            console.log('✅ Grid bot created successfully!');
            ...
            if (data.trailing_up) {
                message += `✓ Trailing Up enabled<br>`;
            }
            if (data.grid_trigger_price) {
                message += `✓ Trigger Price: $${data.grid_trigger_price.toLocaleString()}<br>`;
            }
            if (data.take_profit_pct) {
                message += `✓ Take Profit: ${data.take_profit_pct}%<br>`;
            }
            if (data.stop_loss_price) {
                message += `✓ Stop Loss: $${data.stop_loss_price.toLocaleString()}<br>`;
            }
            ...
            // Reload bots table
            console.log('🔄 Reloading grid bots table...');
            loadGridBots();
            ...
            console.log('❌ Grid bot creation failed:', data.error);
            // Show error
        ...
        showNotification('❌ Error creating grid bot: ' + error.message, 'error');
    } finally {
        // Re-enable submit button
        if (submitBtn) {
            submitBtn.disabled = false;
            submitBtn.textContent = '🚀 Create Grid Bot';
...
async function stopGridBot(botId) {
    console.log(`⏹️ Stopping grid bot #${botId}...`);
    ...
    try {
        ...
        if (data.success) {
            ...
            showNotification('✅ Grid bot stopped successfully!', 'success');
            
            // Reload bots table
            loadGridBots();
        } else {
            console.log('❌ Failed to stop grid bot:', data.error);
...
async function deleteGridBot(botId) {
    console.log(`🗑️ Deleting grid bot #${botId}...`);
    ...
    try {
        ...
        if (data.success) {
            console.log('✅ Grid bot deleted successfully');
            showNotification('✅ Grid bot deleted successfully!', 'success');
            
            // Reload bots table
            loadGridBots();
        } else {
            console.log('❌ Failed to delete grid bot:', data.error);
            showNotification('❌ Failed to delete bot: ' + data.error, 'error');
        }
    } catch (error) {
        console.error('❌ Error deleting grid bot:', error);
        showNotification('❌ Error deleting bot: ' + error.message, 'error');
    }
}
...
async function viewBotLevels(botId) {
    ...
    try {
        ...
        if (data.success) {
            ...
            showNotification('❌ Failed to load bot levels: ' + data.error, 'error');
        }
    } catch (error) {
...
function displayGridLevels(botId, levels, stats) {
    ...
    levels.forEach((level, index) => {
        ...
        const statusText = level.is_filled === 1 ? 'Filled ✅' : 'Pending ⏳';
...
async function stopGridBot(botId) {
    ...
    try {
        ...
        if (data.success) {
            showNotification(`✅ Grid bot stopped! Returned: $${data.returned_investment.toLocaleString()}`, 'success');
...
async function loadDCABots() {
    ...
    try {
        ...
        if (data.success) {
            console.log(`✅ Loaded ${data.bots.length} DCA bots`);
            displayDCABots(data.bots);
        } else {
            console.log('ℹ️ No DCA bots found');
        }
    } catch (error) {
        console.error('❌ Error loading DCA bots:', error);
    }
}
...
async function handleCreateDCABot(event) {
    ...
    console.log('📅 DCA Bot Creation Started!');
    console.log('📋 Form submitted, preparing to create DCA bot...');
    ...
    if (amount < 1) {
        console.log('❌ Validation failed: Amount must be at least $1');
        showNotification('❌ Amount must be at least $1', 'error');
        return;
    }
    
    if (totalCycles < 1 || totalCycles > 1000) {
        console.log('❌ Validation failed: Total cycles must be between 1 and 1000');
        showNotification('❌ Total cycles must be between 1 and 1000', 'error');
        return;
    }
    ...
    try {
        ...
        console.log('📡 Sending POST request to /api/dca_bot/create...');
        ...
        if (data.success) {
            console.log('✅ DCA bot created successfully!');
            console.log(`   Bot ID: ${data.bot_id}`);
            
            // Show success message
            const message = `
                ✅ DCA bot created successfully!<br>
                <br>
                Bot ID: #${data.bot_id}<br>
                Symbol: ${symbol}<br>
            ...
            // Reload bots
            console.log('🔄 Reloading DCA bots table...');
            loadDCABots();
        } else {
            console.log('❌ DCA bot creation failed:', data.error);
            showNotification('❌ ' + (data.error || 'Failed to create DCA bot'), 'error');
        }
        
    } catch (error) {
        console.error('❌ Error creating DCA bot:', error);
        showNotification('❌ Error creating DCA bot: ' + error.message, 'error');
    } finally {
        // Re-enable submit button
        if (submitBtn) {
            submitBtn.disabled = false;
            submitBtn.textContent = '📅 Create DCA Bot';
            console.log('🔓 Submit button re-enabled');
        }
    }
}
...
async function runDCACycle(botId) {
    console.log(`▶️ Running DCA cycle for bot #${botId}...`);
    console.log('📡 Sending POST request to /api/dca_bot/${botId}/run_once...');
    ...
    try {
        ...
        if (data.success) {
            console.log('✅ DCA cycle completed successfully!');
            ...
            console.log('❌ DCA cycle failed:', data.error);
            showNotification('❌ ' + (data.error || 'Failed to run DCA cycle'), 'error');
        }
        
    } catch (error) {
        console.error('❌ Error running DCA cycle:', error);
        showNotification('❌ Error running DCA cycle: ' + error.message, 'error');
    }
}
...
async function stopDCABot(botId) {
    console.log(`⏹️ Attempting to stop DCA bot #${botId}...`);
    ...
    console.log('📡 Sending POST request to /api/dca_bot/${botId}/stop...');
    ...
    try {
        ...
        if (data.success) {
            ...
            showNotification('✅ DCA bot stopped!', 'success');
            
            // Reload bots
            console.log('🔄 Reloading DCA bots table...');
            loadDCABots();
        } else {
            console.log('❌ Failed to stop DCA bot:', data.error);
            showNotification('❌ ' + (data.error || 'Failed to stop DCA bot'), 'error');
        }
        
    } catch (error) {
        console.error('❌ Error stopping DCA bot:', error);
        showNotification('❌ Error stopping DCA bot: ' + error.message, 'error');
    }
}
...
async function loadExchangeAccounts() {
    console.log('📡 Loading exchange accounts for dropdowns...');
    ...
    try {
        ...
        if (data.success && data.accounts) {
            console.log(`✅ Found ${data.count} exchange account(s)`);
            ...
            if (data.count === 0) {
                console.log('ℹ️ No exchange accounts linked. Visit /exchanges to add one.');
            }
        } else {
            console.log('ℹ️ No exchange accounts found');
        }
        
    } catch (error) {
        console.error('❌ Error loading exchange accounts:', error);
    }
}
...
async function loadExchangePortfolio(accountId) {
    console.log(`📊 Loading portfolio for exchange account ${accountId}...`);
    ...
    try {
        ...
        // ⚠️ This makes a REAL API call to the exchange!
        const response = await fetch(`/api/exchange/${accountId}/portfolio`);
        const data = await response.json();
        
        if (data.success) {
            console.log('✅ Exchange portfolio loaded');
        ...
        emptyState.innerHTML = `
            ...
            <p style="font-size: 0.875rem; margin-top: 1rem;">
                Possible reasons:<br>
                • Invalid API credentials<br>
                • Exchange API is down<br>
        ...
        showNotification('❌ Failed to fetch exchange data: ' + error.message, 'error');
    } finally {
        loadingDiv.style.display = 'none';
    }
}
...
function displayExchangePortfolio(data) {
    ...
    showNotification(`✅ Loaded ${data.balance_count} balance(s) from ${data.exchange}`, 'success');
}
...
async function executeAITrade() {
    console.log('🤖 Executing AI-driven trade...');
    ...
    if (!exchangeAccountId) {
        showNotification('❌ Please select an exchange account first', 'error');
        return;
    }
    ...
    if (aiTradeBtn) {
        ...
        aiTradeBtn.textContent = '⏳ Executing...';
    }
    ...
    try {
        ...
        if (data.success) {
            ...
            console.log('✅ AI trade result:', data);
        } else {
            showNotification('❌ AI trade failed: ' + data.error, 'error');
        }
        ...
        if (aiTradeBtn) {
            ...
            aiTradeBtn.textContent = '⚡ Execute AI Trade';
        }
    }
}
...
async function syncLatestPrices() {
    console.log('⬇️ Syncing latest prices from exchange...');
    ...
    try {
        ...
        if (data.success) {
            ...
            showNotification('❌ Failed to sync prices: ' + data.error, 'error');
        }
        ...
        if (button) {
            button.disabled = false;
            button.textContent = '⬇️ Sync Latest Prices';
        }
    }
}
...
function switchGridMode(mode) {
    console.log(`🔄 Switching Grid Bot mode to: ${mode}`);
    ...
    if (mode === 'ai') {
        ...
        if (mode === 'popular') {
            ...
            showNotification('⭐ Popular presets coming soon!', 'info');
        }
    }
}
...
async function generateAIGridParameters() {
    console.log('🤖 Generating AI Grid parameters...');
    
    showNotification('🤖 Analyzing market conditions...', 'info');
    ...
    try {
        ...
        if (priceData.success) {
            ...
            showNotification('✅ AI parameters generated! Review and adjust as needed.', 'success');
        } else {
            showNotification('❌ Failed to fetch price data', 'error');
        }
    } catch (error) {
        console.error('Error generating AI parameters:', error);
        showNotification('❌ Error generating parameters', 'error');
    }
}
...
async function autoFillGridPrice() {
    console.log('🔄 Auto-filling Grid price range...');
    ...
    try {
        ...
        if (data.success) {
            ...
            showNotification('✅ Price range auto-filled!', 'success');
        } else {
            showNotification('❌ Failed to fetch price', 'error');
        }
    } catch (error) {
        console.error('Error auto-filling price:', error);
        showNotification('❌ Error auto-filling price', 'error');
    }
}
...
async function recommendDCAParameters() {
    console.log('✨ Recommending DCA parameters...');
    
    showNotification('✨ Analyzing market for optimal DCA strategy...', 'info');
    ...
    try {
        ...
        if (data.success) {
            ...
            showNotification(
                `✅ Recommended: $${recommendedAmount}/${recommendedInterval} for ${recommendedCycles} cycles (Total: $${totalInvestment})`, 
                'success'
            );
        } else {
            showNotification('❌ Failed to fetch price data', 'error');
...
function initializeToolsNavigation() {
    console.log('🔧 Initializing tools navigation...');
    ...
    if (activeNav && activeSection) {
        console.log(`✅ Initial section: ${activeSection.id}`);
    } else {
        console.log('⚠️ No active section found, activating Grid Bots by default');
        switchToolSection('gridBotsSection');
    }
    
    console.log('✅ Tools navigation initialized');
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (profileForm) {
        profileForm.addEventListener('submit', function(e) {
            e.preventDefault();
            console.log('👤 Profile update requested (placeholder)');
            ...
            showNotification('ℹ️ Profile update feature coming soon! This will save your settings.', 'info');
    ...
    if (subscribeBtn) {
        subscribeBtn.addEventListener('click', function() {
            console.log('💳 Subscription requested (placeholder)');
            showNotification('💳 Subscription feature coming soon! Payment integration will be added in a future update.', 'info');
        });
    }
});
...
switchToolSection = function(sectionId) {
    ...
    if (sectionId === 'fearGreedSection' && !loadedSections.fearGreed) {
        console.log('📊 Loading Fear & Greed Index for first time...');
        loadFearGreed();
        loadedSections.fearGreed = true;
    } else if (sectionId === 'marketOverviewSection' && !loadedSections.marketOverview) {
        console.log('🌐 Loading Market Overview for first time...');
        loadTopCoins();
...
async function loadFearGreed() {
    ...
    try {
        ...
        if (data.success) {
            console.log(`✅ Fear & Greed: ${data.value}/100 (${data.value_classification})`);
        ...
        cardDiv.innerHTML = `
            <div style="text-align: center; padding: 2rem; color: #ef4444;">
                <p><strong>❌ Failed to load Fear & Greed Index</strong></p>
                <p>${error.message}</p>
                <p style="margin-top: 1rem;">
                    <button onclick="loadFearGreed()" class="btn btn-secondary btn-sm">🔄 Try Again</button>
                </p>
            </div>
        `;
        
        showNotification('❌ Failed to load Fear & Greed Index', 'error');
    }
}
...
async function loadTopCoins() {
    console.log('🌐 Loading Top 100 Cryptocurrencies...');
    
    const tbody = document.querySelector('#topCoinsTable tbody');
    
    if (!tbody) {
        console.log('❌ Top coins table not found');
        return;
    }
        ...
        tbody.innerHTML = `
            <tr>
                <td colspan="8" style="text-align: center; padding: 2rem; color: #ef4444;">
                    <p><strong>❌ Failed to load market data</strong></p>
                    <p>${error.message}</p>
                    <p style="margin-top: 1rem;">
                        <button onclick="loadTopCoins()" class="btn btn-secondary btn-sm">🔄 Try Again</button>
                    </p>
                </td>
            </tr>
        `;
...
function displayTopCoins(coins) {
    ...
    console.log('✅ Market overview table populated');
}
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 0.75rem; margin-bottom: 0.75rem;">
                ...
                <div>
                    ...
                    <div style="font-weight: 600; font-size: 0.875rem;">
                        ${ctx.trend === 'long_term_uptrend' ? '📈 Uptrend' : 
                          ctx.trend === 'long_term_downtrend' ? '📉 Downtrend' : '⚪ Sideways'}
                    </div>
                </div>
            </div>
...
async function executeGridBotOnce(botId, exchangeAccountId) {
    try {
        console.log(`🤖 Executing Grid Bot #${botId}...`);
        ...
        console.error('❌ Error executing grid bot:', error);
...
async function executeDCABotOnce(botId) {
    try {
        console.log(`🤖 Executing DCA Bot #${botId}...`);
        ...
        console.error('❌ Error executing DCA bot:', error);
        alert(`Error: ${error.message}`);
        return null;
    }
}
