...
def get_ma_signals(df: pd.DataFrame) -> dict:
    ...
    ema9 = df['ma9'].iloc[-1]
    ema20 = df['ma20'].iloc[-1]
    ema50 = df['ma50'].iloc[-1]
    ema200 = df['ma200'].iloc[-1]
    
    # Get previous bar EMAs for crossover detection
    prev_ema50 = df['ma50'].iloc[-2]
    prev_ema200 = df['ma200'].iloc[-2]
    prev_ema9 = df['ma9'].iloc[-2]
    prev_ema20 = df['ma20'].iloc[-2]
    ...
    if golden_cross:
        explanation_parts.append("🌟 Golden Cross detected! EMA50 crossed above EMA200 (major bullish)")
    elif death_cross:
        explanation_parts.append("💀 Death Cross detected! EMA50 crossed below EMA200 (major bearish)")
    else:
        if trend_label == "long_term_uptrend":
            explanation_parts.append("📈 Long-term uptrend (EMA50 > EMA200)")
        else:
            explanation_parts.append("📉 Long-term downtrend (EMA50 < EMA200)")
    
    if short_term == "bullish":
        if short_cross:
            explanation_parts.append("🔥 Short-term bullish cross (EMA9 > EMA20)")
        else:
            explanation_parts.append("✓ Short-term bullish (EMA9 > EMA20)")
    else:
        if short_cross:
            explanation_parts.append("❄️ Short-term bearish cross (EMA9 < EMA20)")
        else:
            explanation_parts.append("✗ Short-term bearish (EMA9 < EMA20)")
    
    if ema9 > ema20 > ema50 > ema200:
        explanation_parts.append("💪 Perfect bull alignment")
    elif ema9 < ema20 < ema50 < ema200:
        explanation_parts.append("⚠️ Perfect bear alignment")
    
    explanation = " | ".join(explanation_parts)
    
    return {
        'trend_label': trend_label,
        'golden_cross': bool(golden_cross),  # Convert numpy bool_ to Python bool
        'death_cross': bool(death_cross),    # Convert numpy bool_ to Python bool
...
def calculate_indicators(price_rows):
    ...
    Example:
        ...
        print(f"EMA 20: ${indicators['ma20']}")
    """
    
    # Check if we have data (need more for EMA 200)
    if not price_rows or len(price_rows) < 200:
        print(f"⚠️  Not enough price data for indicators (need 200, have {len(price_rows) if price_rows else 0})")
        return None
    ...
    # MA 9 (Very short-term, used by scalpers and day traders)
    # Usage: Price crosses above MA9 = immediate bullish signal
    df['ma9'] = sma(df['close_price'], 9)
    
    # MA 20 (Short-term trend)
    # Usage: If price > MA20, short-term uptrend; if price < MA20, downtrend
    df['ma20'] = sma(df['close_price'], 20)
    
    # MA 50 (Medium-term trend)
    # Usage: When MA20 crosses above MA50, bullish signal (Golden Cross)
    #        When MA20 crosses below MA50, bearish signal (Death Cross)
    df['ma50'] = sma(df['close_price'], 50)
    
    # MA 100 (Long-term trend)
    # Usage: Price holding above MA100 = strong uptrend
    df['ma100'] = sma(df['close_price'], 100)
    
    # MA 200 (Major trend and psychological level)
    # Usage: Most important MA - acts as strong support/resistance
    #        Price above MA200 = bull market
    #        Price below MA200 = bear market
    df['ma200'] = sma(df['close_price'], 200)
    ...
    ema9 = latest['ma9'] if not pd.isna(latest['ma9']) else None
    ema20 = latest['ma20'] if not pd.isna(latest['ma20']) else None
    ema50 = latest['ma50'] if not pd.isna(latest['ma50']) else None
    ema100 = latest['ma100'] if not pd.isna(latest['ma100']) else None
    ema200 = latest['ma200'] if not pd.isna(latest['ma200']) else None
    ...
    result = {
        ...
        'ma9': float(ema9) if ema9 is not None else None,
        'ma20': float(ema20) if ema20 is not None else None,
        'ma50': float(ema50) if ema50 is not None else None,
        'ma100': float(ema100) if ema100 is not None else None,
        'ma200': float(ema200) if ema200 is not None else None,
        'rsi14': float(rsi14) if rsi14 is not None else None,
        'data_points': len(df),
        'timestamp': str(latest['timestamp']) if 'timestamp' in latest else None,
        'ma_chart': ma_chart  # TASK 41: Chart data for visualization
    }
    ...
    if ema9 is not None and ema20 is not None and ema50 is not None:
        if ema9 > ema20 > ema50:
            result['ma_trend'] = 'Strong Uptrend'
            result['ma_color'] = 'darkgreen'
        elif ema9 > ema20:
            result['ma_trend'] = 'Bullish'
            result['ma_color'] = 'green'
        elif ema9 < ema20 < ema50:
            result['ma_trend'] = 'Strong Downtrend'
            result['ma_color'] = 'darkred'
        elif ema9 < ema20:
            result['ma_trend'] = 'Bearish'
            result['ma_color'] = 'red'
        else:
            result['ma_trend'] = 'Sideways'
            result['ma_color'] = 'gray'
    ...
    try:
        ...
        print(f"✅ MA Signals: {ma_signals['overall_signal']} ({ma_signals['confidence']}% confidence)")
    except Exception as e:
        print(f"⚠️  Could not generate EMA signals: {e}")
        result['ma_signals'] = {
            'trend_label': 'unknown',
            'golden_cross': False,
            'death_cross': False,
...
def calculate_simple_indicators(symbol, limit=250):
    ...
    Data Flow (TASK 47):
        CCXT → price_history → unified_data_service → indicators
    ...
    try:
        ...
        if not is_valid:
            print(f"❌ Indicator validation failed: {error_msg}")
            return None
...
→ Price is above moving average → Uptrend
...
Example:
RSI = 75 → Overbought → Price might fall soon → Consider selling
RSI = 25 → Oversold → Price might rise soon → Consider buying
...
⚠️ Note: No indicator is 100% accurate!
